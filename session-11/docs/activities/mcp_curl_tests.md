...
```bash
curl -s "$MCP" -H "Content-Type: application/json" -H "$ACCEPT" -H "$PROTO" -H "Mcp-Session-Id: $SESSION" -d '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name": "celsius_to_fahrenheit_celsius_to_fahrenheit_post","arguments":{"celsius":25}}}'
```
...
```bash
curl -s "$MCP" -H "Content-Type: application/json" -H "$ACCEPT" -H "$PROTO" -H "Mcp-Session-Id: $SESSION" -d '{"jsonrpc":"2.0","id":9,"method":"prompts/get","params":{"name":"explain_conversion",
    "arguments": {
        "input_value": "100",
        "input_unit": "celsius",
...
```bash
# Miles -> km with token JSON Body
curl -s -X POST "$BASE/miles-to-kilometers" \
  -H "Content-Type: application/json" \
  -H "$AUTH" \
  -d '{"miles": 3.1}'

# Celsius -> Fahrenheit
curl -s -X POST "$BASE/celsius-to-fahrenheit?celsius=25" -H "$AUTH"
```
  ...
  "data": {
    ...
    "result": {
      "content": [
        {
          "type": "text",
          "text": "Unknown tool: 'celsius_to_fahrenheit_celsius__post'"
        }
      ],
      "isError": true
    }
